...
echo $METRIC_API_HOST > /root/METRIC_API_HOST

echo $ENVIRONMENT > /root/ENVIRONMENT
echo "* * * * * root PYTHONPATH=/usr/local/lib/python3.7/site-packages/ AWS_CONTAINER_CREDENTIALS_RELATIVE_URI=`cat /root/AWS_CONTAINER_CREDENTIALS_RELATIVE_URI` RABBIT_MQ_URL=`cat /root/RABBIT_MQ_URL` RABBIT_MQ_USERNAME=`cat /root/RABBIT_MQ_USERNAME` RABBIT_MQ_PASSWORD=`cat /root/RABBIT_MQ_PASSWORD` METRIC_API_HOST=`cat /root/METRIC_API_HOST` /usr/local/bin/python /code/app.py >> /var/log/cron.log 2>&1
